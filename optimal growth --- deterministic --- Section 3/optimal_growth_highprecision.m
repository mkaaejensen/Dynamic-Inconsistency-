...
delete(F);
% Figure generation code is embedded in this script - no external figure scripts needed
...
loadoutput  = 0;                    % 0: initiates from horizontal policy; 1: load opts from output.mat with handle specified next
    Handle = 'optimal_growth0_95_highprecision';  % Appended to save and load filenames (saves
...
beta         = 0.95;                % Quasi-hyperbolic discount factor
...
b           = 5;                    % Upper bound for assets (best practice is to compute larger interval than interval of interest)
    bspecfig    = 4;              % Separate upper bound for Figure 1 (due to potential imprecisions near upper bound)
...
extrads     = 0;                    % Full suite of inequality statistics (not used in optimal growth)
extratailm  = 0;                    % Full tail analysis (not used in optimal growth)
...
gridprec    = 500;                   % Grid intervals per unit of assets
dsp         = 1;                    % Second-mover direct search precision (if increased to 5 or 10, finds Krusell-Smith solutions)
innerstep   = 0.1 / sqrt(gridprec*(b-a)); % Initial step length for CMA-ES
tol         = 1e-10;                 % Tolerance (value-function iteration)
...
shownaive  = 0;                     % Shows naive policy in main plot
