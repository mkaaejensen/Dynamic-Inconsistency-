...
loadoutput  = 0;                    % 0: initiates from horizontal policy; 1: load opts from output.mat with handle specified next
    Handle = 'optimal_growth10it';  % Appended to save and load filenames (saves
...
risk_aver    = 1;                   % CRRA parameter (risk aversion)
...
%% Choice of algorithm
algorithm   = 'standard';           % Options:
                                   % 'standard'  %   No policy improvements (for 10 "clean" CMA-ES steps)
...
dsp         = 4;                    % Second-mover direct search precision (if increased to 5 or 10, finds Krusell-Smith solutions)
...
%% Display options
%showhisto  = 1;                    % =1 shows asset distribution
...
if loadoutput == 1
    ...
    randn = 0.5 + 0.5 * rand;
    opts = randn*linspace(a, b, n+1);  % Random initial policy
end
...
if strcmp(algorithm, 'ssr')
    ...
    if beta ==1
        ...
        disp(['Initiating ' algorithm ' CMA-ES (10 iterations, random seed), and computing Markov policy with beta = ' num2str(beta) ' (dynamic inconsistency)']);
    end
...
if risk_aver == 1 && depr == 1  
    ...
    %else
    %    legend([h3,p3], 'location', 'northwest','FontSize',16, 'Interpreter', 'latex'); 
    %end
else
