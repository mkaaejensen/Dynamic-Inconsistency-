function [opts] = algorithm_call_demo(a, b, n, MaxIt, innerstep, opts, RESTART, dissaving, dis, p, u, tol, f, offset,dsp)
...
if RESTART == 2
    ...
    if RESTART == 1
        ...
        %disp([num2str(td) ' Policy Iteration Step(s) Taken...']);
    end
...
for g = 1:MaxIt
    ...
    parfor i = 1:lambda
        ...
        %    [~, Br] = Lossfunction(a, b, n, transformCandidate(BestSol.Position, a, b), ...
                                    % speedy, dis, u, p, 0, tol, f, offset,dsp);
         %   cand = approximateInverseTransform(Br, a, b);
        %    step_i = zeros(1, nVar);
        %else
            step_i = stepsAll(i,:);
