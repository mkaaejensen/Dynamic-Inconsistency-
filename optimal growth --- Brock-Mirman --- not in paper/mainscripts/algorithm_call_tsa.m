function [opts] = algorithm_call_tsa(a, b, n, MaxIt, innerstep, opts, RESTART, dissaving, dis, p, u, tol, f, offset,dsp,tola)
...
%   opts        - updated policy vector after algorithm convergence
    ...
    %% CMA-ES Variant (unused string, but kept for compatibility)
    cmaes_variant = 'active';
    ...
    % Re-interpolate (if grid changes at restart)
    opts = interp1(linspace(a, b, length(opts)), opts, G, 'linear');
    ...
    pc{1} = zeros(VarSize);
    sigma{1} = sigma0;
    % Bias covariance matrix in direction of increasing solutions.
    bias_factor = 0.5;
    ...
    for g = 1:MaxIt
        ...
        if stall_counter >= 5   % note that this is very aggressive (consider disabling entirely on powerful processors, e.g. set to 100)
