function [opts] = algorithm_call_default(a, b, n, MaxIt, innerstep, opts, RESTART, dissaving, dis, p, u, tol, f, offset,dsp,tola)
% CMA-ES algorithm 
% See repository documentation for full documentation ("DOCU") of transformations 
% (ztox and xtoz) and lossfunction. This implementation is standard CMA-ES
% (DOCU.6.3.1) with periodic best response injection into the CMA-ES pool (if (i == specialCandidateIndex)  below; DOCU.6.4.1)
%
wtc= 0; % writes additional information to the consolue
WB = 1; % enables waitbar          
...
G = linspace(a, b, n+1);
opts=interp1(linspace(a, b, length(opts)), opts, G, 'linear');  % for restart with different grid
opts=max(min(opts, b), a);                                      % safeguard against interpolation errors

% RESTART == 1: Supervised policy iteration to generate initial candidate
% for CMA-ES
if RESTART == 1
     for m = 1:100
         [~, Br] = Lossfunction(a, b, n, opts, dissaving, dis, u, p, 0, tol, f, offset,dsp);
         opts2 = Br(:);% max(min(transpose(Br), min(f(G), b)), a);
...
z_init = xtoz(opts, a, b); % transform intial candidate to z space (from now on everything runs in z space)

% CMA parameters (refer to reference in DOCU.6.3.1)
lambda = 8;  % on a powerful processor change to (4 + round(3*log(nVar))) * 2; 
mu = round(lambda/2);
...
for g = 1:MaxIt
    ...
    % cell array for candidate results (emptied at each iteration as safeguard )
    pop_temp = cell(lambda, 1);
    
    % Parallel candidate evaluation loop
    parfor i = 1:lambda
        if (i == specialCandidateIndex) % periodically inject policy iteration into candidate pool
            [~, Br] = Lossfunction(a, b, n, ztox(BestSol.Position, a, b), ...
                                     dissaving, dis, u, p, 0, tol, f, offset,dsp); % get best response
            cand = xtoz(Br, a, b); %transform best response to z-space and inject 
            step_i = zeros(1, nVar); % reset associated step without refinement
        else
            step_i = stepsAll(i,:);
    ...
    if stall_counter >= 20
        innerstep = innerstep/2;                    % very aggressive (reduce to e.g. 0.9 or disable entirely on powerful processors)
        sigma{g+1} = innerstep;
        C{g+1} = eye(nVar);
...
function z = xtoz(x, a, b)
    ...
    % first coordinate (transforms with 1e-14 to prevent singularities/zeroes)
