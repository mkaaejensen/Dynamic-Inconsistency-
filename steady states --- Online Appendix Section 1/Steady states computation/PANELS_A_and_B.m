...
% or non-recursive. The recursive case is uniquely given by the discount
...
PaRRA=2;                        % Panel A Rate of Risk Aversion
RRA=1;                          % Panel B Rate of Risk Aversion
rl=1.85; rh=2.12;                % Range in figures (imposed for easy comparison). To disable set r1=-1000;
epsilon=0.01;                  % step length in search for zero (note that this is set slightly high for visual reasons, reduce for greater accuracy)
[comss comslope compstrat]=parent_panel_a(PaRRA,dis,cutoff,rl,rh,epsilon); % computing comparison ss and slope
...
for jj=1:1:num;
     ...
     if flag==0 & jj>1 & prev*curr<=0;
         sol=scs(jj-1);  % note: Unless grid precision is very high, both jj and jj-1 are defensible here.
         slope=tt(jj-1);
         flag=1;
     end;
        prev=curr;
...
hold on;
plot(tt,flat,'--','color','k','Linewidth',2,'HandleVisibility','off');
hold on;
...
if RRA==1;
    kss=((caps*beta*delta*A)/(1-caps*delta*(1-beta)))^(1/(1-caps)); % analytical solution
    n=@(a) (1-delta*beta*Df(a))/(delta*(1-beta)); % slope of strategy at steady state a in K-S model
    display(['Since RRA = 1, we know an analytical solution which for comparison is ' num2str(kss) ' with slope ' num2str(n(kss))]);
end;
display(['In Panel B, the marginal utility of a+e at the Panel A steady state is: ' num2str(V(comss,comslope))]);
display(['In Panel B, the actual support slope of a+e at the panel A steady state is ' num2str(double(fminbnd(@(x) abs(V(comss,x)),0,1)))]);
display(['But given this slope (and the panel A ss), in Panel B, the marginal utility at the ss (at a) is ' num2str(W(comss,double(fminbnd(@(x) abs(V(comss,x)),0,1))))]);
display(['Contour directions (panel B):']);
