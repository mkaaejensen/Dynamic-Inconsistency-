...
dis=dis(1:1:cutoff);            % Discount sequence d(t)
...
%% Analytical RCE steady state from theorem: R(k*) = 1 + 1 / sum_t d(t)
A1      = sum(dis);                    % Approximate sum_{t=1}^\infty d(t)
Rstar   = 1 + 1/A1;                    % Theoretical R(k^*) from Theorem
fun_kR  = @(k) (R(k) - Rstar).^2;      % Invert R(k) = Rstar
kRCE_th = fminbnd(fun_kR, 1e-6, xmax, opts);

%% Print steady states  
fprintf('Planner steady state:          k^* = %.10f, s = %.10f\n',solSP,slopeSP);
fprintf('RCE steady state:              k^* = %.10f, s (private)* = %.10f\n',solR,slopeR);
fprintf('RCE steady state:              k^* = %.10f\n',kRCE_th);
