...
for jj=1:1:num
    funR=@(x) abs(V_R(x,tt(jj),epsilon,Du,f,Df,dep,dis,cutoff));
    funcR=@(x) abs(W_R(x,tt(jj),Du,f,Df,dep,dis,cutoff));
    scsR(jj)=double(fminbnd(funR,min,max));
    scscR(jj)=double(fminbnd(funcR,min,max));
    margR(jj)=V_R(scsR(jj),tt(jj),epsilon,Du,f,Df,dep,dis,cutoff);
    margcR(jj)=W_R(scscR(jj),tt(jj),Du,f,Df,dep,dis,cutoff);
...
%% Plot Panel A (RCE)
figure('Name', 'Panel A (RCE)');
h1R=plot(tt,scscR,'-','color','#800000','Linewidth',3);
hold on;
h2R=plot(tt,scsR,'-','color','#000080','Linewidth',3);
hold on;
...
function valR = V_R(a,s,epsilon,Du,f,Df,dep,dis,cutoff)
    ...
    e=epsilon;
    % F_y at (k^*+ε, k^*+sε)
    c0 = Rloc(a+e).*(a+e) + wloc(a+e) - (a + s*e);
    Fy0 = -Du(c0);
    valR = Fy0;
    for jv=1:1:cutoff
        k_state = a + (s^jv)*e;
        y_next  = a + (s^(jv+1))*e;
        c = Rloc(k_state).*k_state + wloc(k_state) - y_next;
        Fx = Rloc(k_state).*Du(c);
        Fy = -Du(c);
