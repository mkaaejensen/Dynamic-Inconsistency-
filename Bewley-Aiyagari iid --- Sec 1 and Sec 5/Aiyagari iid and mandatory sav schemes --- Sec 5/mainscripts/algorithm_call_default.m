function [opts] = algorithm_call_iid(a, b, n, MaxIt, innerstep, opts, RESTART, dissaving, dis, p, u, tol, f, offset, dsp, dyncon, sm,tola)
...
% Store true initial loss for reporting
...
% Hansen: explicit mean vector and separate sigma path
m = z_init;
p_sigma = zeros(1, nVar);
C = eye(nVar);
chol_counter = 0;
...
x = dyncon;
x(idxA) = xfromz(current_pos, a, b, nA-1);
...
% Preallocate arrays for parfor efficiency
...
for g = 1:MaxIt
    ...
    if g > 1
        if BestCost(g)/nVar<tola
                disp(['Ego Loss equals zero within tolerance (' sprintf('%.5e', BestCost(g)/nVar) '). Finishing']);
        ...
        elseif WB==1
            waitbar((g-1)/MaxIt, hWaitbar, ...
                ['Ego Loss = ' sprintf('%.5e', BestCost(g-1)/nVar) ...
                 ' (Iteration ' num2str(g-1) ' of ' num2str(MaxIt) ')']);
        end
    end

    if g == MaxIt
        disp(['Iteration ' num2str(g) ' [final] Ego Loss = ' sprintf('%.5e', BestCost(g)/nVar)]);
        if WB==1, close(hWaitbar); end
        break;
    end

    % Hansen: sort once, then weighted sum
    [sorted_costs, sorted_indices] = sort(candidate_costs);
    weighted_step = zeros(1, nVar);
    for jj = 1:mu
        weighted_step = weighted_step + w(jj) * candidate_steps(sorted_indices(jj),:);
    end
    
    % Hansen: m ← m + σ⟨y⟩
    m = m + sigma * weighted_step;
    current_pos = m;
    ...
    % Hansen: separate sigma evolution path
    C_inv_sqrt_step = weighted_step / A;
    p_sigma = (1-cs)*p_sigma + sqrt(cs*(2-cs)*mu_eff)* C_inv_sqrt_step;
    sigma = sigma * exp(cs/ds*(norm(p_sigma)/ENN - 1));
    ...
    % Hansen: full covariance matrix update
    C = (1 - c1 - cmu + delta_c) * C + c1 * (pc' * pc);
    for jj = 1:mu
        step_j = candidate_steps(sorted_indices(jj),:);
        C = C + cmu * w(jj) * (step_j' * step_j);
    end
    
    % Hansen: periodic Cholesky decomposition
    chol_counter = chol_counter + 1;
    if chol_counter >= 10
        A = chol(C)';
        chol_counter = 0;
    end
...
% Final solution extraction (removes artificial increase due to
% transformation)
x = dyncon;
x(idxA) = xfromz(best_pos, a, b, nA-1);
...
% Inverse mapping: weakly non-decreasing sequence in [a,b] to z ∈ R^{active_dims}
