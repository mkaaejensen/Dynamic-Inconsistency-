...
% Means
MEAN  = S.MA;%mean(S.asim(:))/mean(S.lsim(:));
MEANK = S.MAK;%mean(S.asimK(:))/mean(S.lsimK(:));
...
disp(['1st asset decile QH: ' num2str(quantile(asim, 0.1))]);
disp(['1st asset decile Aiyagari: ' num2str(quantile(asimK, 0.1))]);
disp(['2nd asset decile QH: ' num2str(quantile(asim, 0.2))]);
disp(['2nd asset decile Aiyagari: ' num2str(quantile(asimK, 0.2))]);
borrow_lim=S.a;
if S.extrads == 1
    % -------------------------------------------------------------------------
...
fprintf('Borrowing constrained  QH: %.3f%% | A: %.3f%%\n',constraint_mass_QH,constraint_mass_A);
...
% 1) Bar for the spike at a = 0
%bar(S.a, massQH0, bin_width, 'FaceColor', nred,  'EdgeColor','none');
%bar(S.a, massA0,  bin_width, 'FaceColor', nblue, 'EdgeColor','none');
