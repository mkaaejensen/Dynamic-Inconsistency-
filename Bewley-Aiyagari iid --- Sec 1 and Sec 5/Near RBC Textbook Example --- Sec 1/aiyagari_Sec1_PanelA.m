...
sm = 0; % no saving mandate case (Panel A).  

%% Preferences 
delta     = 0.95;            % Discount factor
    calmode      =0;         % If =1, QH and DC computations use different discount factors (for calibrations)
        deltaDC     = delta; % DC discount factor if calmode = 1
beta       = 0.88;           % = 1 => DC solution
risk_aver = 1;         

if beta == 1 % no mandate imposed on DC solution (for consistency)
    sm = 0;
end
%% Initiation
loadoutput  =  0;     % 0: horizontal policy; 1: load precomputed solution (Handle below)
    Handle = 'intro_fig_panel_A'; % Appended to save and load (can be left empty/%'ed out leading to repeated overwrite)
figonly     = 0;      % =1 draws figures from precumputed solution (loaded from Handle)
shortv      = 0;      % breaks after first figure (when figonly = 0)
