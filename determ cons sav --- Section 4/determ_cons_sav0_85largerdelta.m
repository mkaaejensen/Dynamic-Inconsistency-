...
Handle = mfilename;%'deter_cons_save0_85largerdelta';  % Appended to save and load (can be left empty/%'ed out leading to repeated overwrite)
...
ssrdc   = 0;          % if = 1, algorithm is used to compute DC and naive solution in ssr case. If = 0, DC computed via Kaplan's EGP algorithm
dcVkaplan = 1;        % overwrites all other dynamically consistent computations with EGP (Kaplan) version 
...
u = @(c) u(c)-u(f(a)-a)+1; % scaling utility (improves numerical accuracy for welfare computations) 

if exist('r', 'var')
    disp(['Interest rate: ' num2str(100*r) '%, modified golden rule interest rate ' num2str(100*(1/delta-1)) '%, Wage: ' num2str(W) ]);
    disp(['Borrowing limit: ' num2str(a) ', Consumption at borrowing limit ' num2str(f(a)-a) ', Natural borrowing limit: ' num2str(-((W*lmin)/(r)))]);
end
