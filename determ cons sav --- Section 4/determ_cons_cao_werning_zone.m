...
Handle = 'deter_cons_cwz';  % Appended to save and load (can be left empty/%'ed out leading to repeated overwrite)
...
algorithm   = 'ssr';
                        ...
                        % 'tsa'         %   Hybrid CMA-ES-policy iteration
                                        %   algorithm (sometimes very effective in stochastic models and is preferred in dynamically
                                        %   consistent models)
dissaving = 1;        % if = 1, second-mover is limited to weak dissaving policies in Lossfunction (improves speed and accuracy), if =2 the same is true and extra numerical check is made against 45 degree line (important if there are steady states, otherwise irrelevant). Imposed by default below if interest is in relevant range.
...
%% prices (overwrite) 1/(1+inter(kss));
r = 1/delta-1+0.001;  
W = 5.674; 
