...
cutoff = 800;                   % cutoff for truncated value function approximation
delta     = 0.951;              
beta      = 0.93; 
 
%s=1.00;
 
kpar = 0.083; % We set the hyperbolic discount parameter $\kappa = 0.083$, which ensures the hyperbolic and geometric models have equivalent total discounting over economically relevant horizons. Specifically, the average per-period discount factor in the hyperbolic model equals $[1/(1+\kappa T)]^{1/T}$. With $\kappa = 0.083$, this yields: 20-period average: 0.952; 50-period average: 0.961; 100-period average: 0.969; 200-period average: 0.975. Our geometric discount factor $\delta = 0.951$ falls at the lower end of this range, with the models having nearly identical average discounting over a 20-period horizon. While the hyperbolic model front-loads discounting (creating present bias with immediate discount factor 0.923), both models generate similar total discounting over short to medium horizons.
 
dis = @(j) 1./(1+kpar.*j);              % hyperbolic (one parameter)
%dis = @(j) 1 ./ (1 + kpar .* j).^s;        % generalized hyperbolic
...
loadoutput  =  0;     % 0: horizontal policy; 1: load opts from output.mat; etc. 2: initiate from DC in dyncon
Handle = 'deter_cons_sav_true_hyp';  % Appended to save and load (can be left empty/%'ed out leading to repeated overwrite)
...
ssrdc   = 1;          % if = 1, algorithm is used to compute DC and naive solution in ssr case. If = 0, DC computed via Kaplan's EGP algorithm
dcVkaplan = 0;        % overwrites all other dynamically consistent computations with EGP (Kaplan) version 
...
showSM     = 0;    % shows second-mover in main plot (only low productivity)
...
% note that offset is not relevant in deterministic models
offset = @(ass) ass+(lmax-lmin)*W/(1+r); % consumption-saving model offset


if exist('r', 'var')    
    disp(['Interest rate: ' num2str(100*r) '%, modified golden rule interest rate ' num2str(100*(1/delta-1)) '%, Rstar (Cao-Werning Threshold) ' num2str(100*((1-delta)/(beta*delta))) '%, Wage: ' num2str(W) ]);
    ...
    if (r >= (1/delta - 1)) && (r <= (1 - delta)/(beta*delta))
        dissaving = 2; % tells ssr algorithm to perform an additional precision check near 45 degree line
    end
end
...
if loadoutput == 1
    if exist(optssave, 'file') == 2
        ...
        disp(['No ' optssave ' file found. Initiating from horizontal policy at a']);
        opts = a*ones(1, n+1);
        RESTART=1;
    end
else
...
if strcmp(algorithm, 'ssr')
    ...
    cct   = 1;  
    con   = 0; % = 0 uses constant policy for continuations =1 uses opts
    branch = 1; % = 0 uses value-function iteration, = 1 uses summation
    disp(['Computing Hyperbolic model with hyperbolic parameter  = ' num2str(kpar)]);
    ...
    if ssrdc==1
        ...
        [~, dyncon, ~] = algorithm_ssr(a, b, n, u, disdc, cct, opts, dissaving, f, offset, p, tol, con, branch);
    ...
    if beta ==1
        disp(['Initiating ' algorithm ' CMA-ES and computing Markov policy with beta = ' num2str(beta) ' (dynamical consistency)']);
    else
        disp(['Initiating ' algorithm ' CMA-ES and computing Markov policy with beta = ' num2str(beta) ' (dynamical inconsistency)']);
    end; 
    ...
    opts = algorithm_handle(a, b, n, MaxIt, innerstep, opts, RESTART, dissaving, dis, p, u, tol, f, offset,dsp);
    disp('Computing naive solution')
...
elseif figonly == 0
    save(optssave,'opts');
    if strcmp(algorithm, 'ssr') && ssrdc==1;
    save(dynconsave,'dyncon');
    end;
end
if figonly == 1
...
drawnow
    ...
    if showkaplan==1 && ~strcmp(algorithm, 'ssr')
